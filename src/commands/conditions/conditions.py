...
class Conditions(IConditions):
  ...
  def __init__(self, complex_strategy: IComplexCondition, simple_strategy: ISimpleCondition) -> None:
  ...
  def execute(self, conditions: str) -> dict:
    try:      
      ...
      for condition in conditions:        
        strategy = self.complex_strategy if condition["name"] in self.complex_strategy.operations else self.simple_strategy
        sql += " AND " if sql != "" else ""
